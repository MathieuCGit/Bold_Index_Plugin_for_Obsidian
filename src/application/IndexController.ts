...
// Called when the user clicks a line number inside the index. It must move the editor selection to that bold occurrence.
...
export class IndexController {
  ...
  constructor(private readonly app: any) {
    // The view is responsible for drawing the list and the search input.
    this.view = new IndexView(this.app);
  }

  // Reads the current note, builds the bold index, and forwards the result to the view.
  async render(container: any, onNavigate: IndexNavigationHandler): Promise<void> {
    ...
    // Read the raw markdown content so the parser can extract bold terms and their positions.
  ...
  // Convenience method for tests and reuse: returns the same bold index as the parser itself.
